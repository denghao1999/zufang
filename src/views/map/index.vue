<template>
  <div>
    ...
    <div id="container">
     <!-- 导航栏 -->
    <van-nav-bar title="地图找房" class="page-nav-bar">
...
export default {
  ...
  mounted () {
    ...
    // // 初始化地图，设置中心点坐标和地图级别
    const { BMapGL } = window
    var myCity = new BMapGL.LocalCity();//获取当前电脑所在的城市
    myCity.get(function (result) {
      console.log(result);//杭州
      var map = new BMapGL.Map("container");
      var point = new BMapGL.Point(result.center.lng, result.center.lat);//绘制地图
      map.centerAndZoom(point, 15);//根据某城市的中心点绘制图 15是缩放级别


      // 每个区在区的中心点画一个标记
      var point1 = new BMapGL.Point(116.449979, 39.912338);//经纬度在接口中通过城市名字获取
      var content = '朝阳';
